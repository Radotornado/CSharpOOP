...
public static class CommandExecutor
{
    ...
    public static void Analyze(List<HardwareComponent> computer)
    {
        ...
        Console.WriteLine("Total Operational Memory: {0} / {1}", computer.Select(h => h.UsedMemory).Sum(),
            computer.Select(h => h.MaximumMemory).Sum());
        Console.WriteLine("Total Capacity Taken: {0} / {1}", computer.Select(h => h.UsedCapacity).Sum(),
            computer.Select(h => h.MaximumCapacity).Sum());
    }
    ...
    public static void SystemSplit(List<HardwareComponent> computer)
    {
        foreach (HardwareComponent hardware in computer.Where(h => h is PowerHardwareComponent))
        {
            ...
            Console.WriteLine("Express Software Components - {0}",
                hardware.SeeSoftware().Where(s => s is ExpressSoftwareComponent).Count());
            Console.WriteLine("Light Software Components - {0}",
                hardware.SeeSoftware().Where(s => s is LightSoftwareComponent).Count());
            ...
            if (hardware.SeeSoftware().Count > 0)
            {
                Console.WriteLine("Software Components: {0}",
                    string.Join(", ", hardware.SeeSoftware().Select(s => s.Name)));
            }
            else
    ...
    public static void DumpAnalyze(List<HardwareComponent> computer, List<HardwareComponent> dump)
    {
        ...
        Console.WriteLine("Express Software Components: {0}",
            dump.Select(h => h.SeeSoftware().Where(s => s is ExpressSoftwareComponent)).Count());
        Console.WriteLine("Light Software Components: {0}",
            dump.Select(h => h.SeeSoftware().Where(s => s is LightSoftwareComponent)).Count());
